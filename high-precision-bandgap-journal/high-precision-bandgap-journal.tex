...
\documentclass{article} % which different types of dodcuments are there?
...
% difference between fontenc and inputenc?
...
\begin{abstract}
	Includes specifications for the block. Different topologies for Bandgap and Operational Transconductance Amplifier.

	\textbf{Keywords:} high precision, trimming, Kuijk, Brokaw, Tsividis, bandgap, voltage reference, OTA
\end{abstract}

\section{Topology classification}
...
\begin{figure}[ht!]
	...
	\label{fig:kuijk} % How to pronaunce!?
\end{figure}

This design is based on Kuijk's paper from 1972.
...
Because of scaling $A_E$, current constants $I_s$ are different.
...
This difference is applied to resistor $R_3$.
...
\newpage % Appendixlike in the middle of doc
...
\begin{equation}
	I_{PTAT} = (1 - \dfrac{1}{\sqrt{N}})^2 \dfrac{1}{R^2} \dfrac{1}{\mu_n C_{ox} (\frac{W}{L})_1}
\end{equation}
...
In Fig.	\ref{fig:PTAT-bias-startup}, Start-up circuit can be seen. Start-up circuit is needed to avoid the zero-current state in the PTAT Bias Circuit. When gates of PMOS mirror are close to $V_{DD}$, and NMOS Mirror gates are close to $GND$. With included start-up circuit $PM_{FB}$ is also zero current, which means ther mirror is off and current through $R_{HI}$ is 0. So on start-up $NM_{STUP}$ is on and droping the voltage on gates of PMOS current. Turning it on will turn on the $PM_FB$, which by feedback turns on the NMOS mirror inside of start-up circuit, starting the current through $R_{HI}$. If that resistance is high enough voltage will drop and $NM_{STUP}$ is turned off, isolating the start-up circuit is disconnected from the PTAT bias.

\section{Bandgap Reference Design}

\begin{figure}[ht!]
	\centering
	\includegraphics[width=\linewidth]{Figures/BGRV-design.pdf}
	\caption{Bandgap Reference Voltage} % Proportional to absolute temperature Bias
	\label{fig:BGRV-design}
\end{figure}

Green nodes in Fig. \ref{fig:BGRV-design} represent nodes that are part of analog test, without it there is 6 dB gain for this frequency range from supply to output reference. Red currents are currents of interest, and $I_{B+}$ is not zero because of in NPN differential pair.

\subsection{Passive Low-Pass Filter}

Purpose is Power Supply Rejection ($PSR^+$) at frequencies higher than $100$ kHz.


% TODO add plot comparing PSR+ with and without \addplot
%% lazy neovim - d$ deletes everything till EOL in Normal Mode
...
% TODO Add functional test with disconnected loop on the outtput of Comparator OTA

\subsection{Buffer OTA}
% Jedinični bafer translate to english.
Expected input is $1.306$ V. Transistor used for input differential pairs is npn BJT.

\subsection{Testing stability} % is part of simulation tasks or functional or behavoural tasks?

Identified three loops: feedback loop for comparator, buffer loop, and start-up circuit. Loop inside of start-up circuit has no gain so it is stable. For the other two Phase Margin and Gain Margin needs to be simulated.

\section{Parameter Definitions}

\subsection{Inaccuracy Absolute Error}

\subsection{Temperature Coefficient}

Is defined by box method.

% TODO update difference between simulation def, lab test def and

\section{Design for Testability}

\begin{itemize}
	\item Analog Test Bus
	\item Analog Stress Test
	\item IDDQ Quiescent Current
\end{itemize}
