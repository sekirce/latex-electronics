...
\begin{abstract}
    Includes specifications for the block.
...
\textbf{Keywords:} high precision, trimming 
\end{abstract}

\section{Schematic for calculation} % Mathematical Behaviour :/

\begin{figure}[ht!]
	\centering
	\includegraphics[width=0.7\linewidth]{Figures/BGRV-Kuijk.pdf}
	\caption{Kuijk Bandgap Reference Voltage}
	\label{fig:kuijk} % !?
\end{figure}

\begin{equation}\label{eq:id1}
    I_{D1} = I_{s1} (e^{V_{BE1}/V_T}-1)
\end{equation}

\begin{equation}\label{eq:id2}
    I_{D2} = I_{s2} (e^{V_{BE2}/V_T}-1)
\end{equation}
...
Divide \ref{eq:id1} and \ref{eq:id2}:
...
\begin{equation}
    \Delta V_{BE} = \dfrac{kT}{q} ln (\dfrac{n I_{D1}}{I_{D2}})
\end{equation}

\begin{equation}
    \dfrac{\Delta V_{BE}}{R_3} = \dfrac{V_{BGR} - V_{BE1}}{R_2}
\end{equation}
